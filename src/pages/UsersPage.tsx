...
export default function UsersPage() {
  ...
  return <div className="page-stack">
    ...
    <Card className="toolbar-card"><SearchBox value={search} onChange={value => { setSearch(value); setPage(1) }} placeholder="نام، شماره همراه یا ایمیل" /><div className="filters"><ProductSelect ariaLabel="فیلتر وضعیت کاربران" value={state} onValueChange={value=>{setState(value);setPage(1)}} options={[{value:'',label:'همه وضعیت‌ها'},{value:'active',label:'فعال'},{value:'banned',label:'مسدود'}]}/><ProductSelect ariaLabel="فیلتر پلن کاربران" value={plan} onValueChange={value=>{setPlan(value);setPage(1)}} options={[{value:'',label:'همه پلن‌ها'},{value:'free',label:'رایگان'},{value:'premium',label:'پریمیوم'}]}/><ProductSelect ariaLabel="مرتب‌سازی کاربران" value={`${sort}:${direction}`} onValueChange={value=>{const[s,d]=value.split(':');setSort(s);setDirection(d);setPage(1)}} options={[{value:'time:desc',label:'جدیدترین'},{value:'time:asc',label:'قدیمی‌ترین'},{value:'name:asc',label:'نام از الف'},{value:'name:desc',label:'نام از ی'}]}/></div></Card>
